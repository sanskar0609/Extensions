              ...
              <div className="nav-card-links mt-auto flex flex-col gap-[2px]">
                {item.links?.map((lnk, i) => (
                  <button
                    key={`${lnk.label}-${i}`}
                    onClick={() => {
                      if (lnk.onClick) lnk.onClick(); // ✅ internal navigation
                      else if (lnk.href) window.open(lnk.href, "_blank"); // ✅ external link
                    }}
                    aria-label={lnk.ariaLabel}
                    className="nav-card-link inline-flex items-center gap-[6px] no-underline cursor-pointer transition-opacity duration-300 hover:opacity-75 text-[15px] md:text-[16px] bg-transparent border-none text-left"
                  >
                    <GoArrowUpRight
